...
# -----------------------------------------------------------------------------
# NVIDIA API Key & Model Configuration
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Initialize LLM via NVIDIA Hosted NIM / API Catalog
# -----------------------------------------------------------------------------
# Temperature is set to 0.3 for consistent classification and controlled responses
llm = ChatNVIDIA(
    model=model_name,
    temperature=0.3,
...
with st.sidebar:
    ...
    st.write("**Two-Chain LangChain Pipeline (LCEL)**")
    ...
    st.write("**Provider:** `NVIDIA Hosted NIM / API Catalog`")
    st.write(f"**Model:** `{model_name}`")
    st.write("**Temperature:** `0.3`")
    st.caption(
        "Temperature 0.3 balances deterministic classification with controlled, "
        "natural language variation for response drafting."
    )
...
st.markdown("A LangChain-powered complaint classification and response generation application powered by NVIDIA NIM.")
st.markdown("---")
...
with st.chat_message("assistant"):
    st.markdown(
        "👋 **Hello! I am your AI Support Assistant for XYZ Finance.**\n\n"
        ...
        "I am here to help you get quick resolution.\n\n"
...
if active_complaint:
    ...
    if not clean_complaint:
        ...
        with st.chat_message("assistant"):
            with st.spinner("Classifying complaint and generating response via NVIDIA NIM..."):
                try:
                    ...
                    if "401" in err_msg or "unauthorized" in err_msg.lower():
                        st.error(
                            "❌ **NVIDIA API Authentication Error**\n\n"
                            "The NVIDIA API key is invalid or unauthorized. Please verify your `NVIDIA_API_KEY`."
                        )
                    elif "429" in err_msg or "rate limit" in err_msg.lower():
                        st.error(
                            "❌ **NVIDIA API Rate Limit Exceeded**\n\n"
                            "Rate limit reached on NVIDIA API Catalog. Please pause briefly before retrying."
                        )
                    else:
                        st.error(f"An error occurred while processing your request: {err_msg}")
